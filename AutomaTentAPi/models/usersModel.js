...
class User {
    ...
    static async register(user) {
        try {
            ...
            await pool.query(`Insert into usr(usr_name, usr_email, usr_pass) values($1,$2,$3)`,
            [user.name, user.email,encpass]);
            dbResult = await pool.query(`Select * from usr where usr_email=$1`,
            [user.email]);
        ...
        try {
            ...
            return { status: 200, result: 
                new User(dbUser.usr_id, dbUser.usr_name, dbUser.usr_email , dbUser.usr_pass, dbUser.usr_token)} ;
        } catch (err) {
            console.log(err);
            return { status: 500, result: err };
        }  
    ...
    static async checkLogin(user) {
        try {
            let dbResult =
                await pool.query("Select * from usr where usr_name=$1", [user.name]);
            let dbUsers = dbResult.rows;
            if (!dbUsers.length)
